...
def calculate_availability_and_losses(
    df=None,
    input_path: str = "data/processed/kelmarsh_consolidated.parquet",
    output_path: str = "data/processed/availability_results.csv",
) -> pd.DataFrame:
    ...
    if df is None:
        ...
        # OPT: cópia apenas quando recebe df externo para proteger o caller
        df = df.copy()
    ...
    # OPT: filtragem com máscara booleana direta, sem .copy() desnecessário
    ...
    df_unavail["Lost_Energy_kWh"] = (
        df_unavail["Duration_Hours"] * df_unavail["Avg_Power_kW"]
    )

    uk_power_prices = {2019: 0.14, 2020: 0.13, 2021: 0.19}
    df_unavail["Price_per_kWh"] = df_unavail["Year"].map(uk_power_prices)
    df_unavail["Financial_Loss_GBP"] = (
        df_unavail["Lost_Energy_kWh"] * df_unavail["Price_per_kWh"]
    )

    results = (
        df_unavail.groupby(["Turbine", "Year"])
        .agg(
    ...
    results["Availability_Percentage"] = (
        results["Available_Hours"] / results["Total_Year_Hours"] * 100
    )
    ...
    round_cols = {
        ...
        "Available_Hours": 2,
        "Lost_Energy_kWh": 2,
        "Financial_Loss_GBP": 2,
        "Availability_Percentage": 2,
    }
    results = results.round(round_cols)
